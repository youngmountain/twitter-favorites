...
TwitterFavorites.prototype.getFavorites = function getFavorites(user) {
  ...
  var starCallback = function(err, res) {
    ...
    this.getFavoritesFromUser(params, starCallback.bind(this));
  };
...
TwitterFavorites.prototype.getFavoritesFromUser = function callStarApi(params, cb) {
  params = params || {};
  ...
  request.get({url: url, oauth: this.oauth, json: true}, function(err, req, body) {
    if (err) {
      return cb(err);
    }
    if (body && body.errors) {
